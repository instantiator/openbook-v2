...
int user_screen_width = 75;
int user_screen_height = 24;
...
void os_process_arguments(int argc, char *argv[]) 
{
    ...
    vis_debug('!', "Incorrect number of arguments.");
	puts(usage);
	exit(1);
    }
    ...
    vis_debug('f', "Story file:");
    vis_debug('-', story_name);
...
void os_init_screen(void)
{
  ...
  /* UPDATE: set ALL OF rows, cols, width, height these here now */
  
  h_screen_rows = user_screen_height;
  h_screen_cols = user_screen_width / os_char_width('.');
                /* represents 'maximum' expected number of columns */
                /* period being the smallest char i could think of */

  h_screen_width = user_screen_width; /* graphical units for h_screen_width */ 
  h_screen_height = user_screen_height; /* cheating, still */
  
  extern FILE* debug;
  fprintf(debug, "[I] Initialising with h_screen_cols  = %d\n", h_screen_cols);
  fprintf(debug, "[I] Initialising with h_screen_width = %d\n", h_screen_width);
