...
static struct {
    ...
    zword x_pos;
    zword y_size;
    zword x_size; /* COLUMNS */
    zword width;  /* GRAPHICAL UNITS */
    zword y_cursor;
    zword x_cursor;
    zword x_current_width; /* WOAH */
    zword left;
    zword right;
    zword nl_routine;
    zword nl_countdown;
...
static void reset_cursor (zword win)
{
    ...
    wp[win].x_current_width = (os_char_width(' ') * wp[win].left) + 1;
...
/*
 * units_left
 * Return the #screen units GRAHPICAL UNITS from the cursor to the end of the line.
 * unit_cols_left
 * Same but COLUMNS
 *
 */
...
static int units_left (void) {
    ...
    extern FILE* debug;
    fprintf (debug, "[u] Units left, vars:      cwp->width    = %d\n", cwp->width);
    fprintf (debug, "                           cwp->r*os_c_w = %d\n", cwp->right*os_char_width(' '));
    fprintf (debug, "                           cwp->x_c_w    = %d\n", cwp->x_current_width+1);
    fprintf (debug, "[U] Units left, returning:               = %d\n", cwp->width - (cwp->right*os_char_width(' ')) - cwp->x_current_width + 1);
    
    return cwp->width - (cwp->right*os_char_width(' ')) - cwp->x_current_width + 1; 
    
}/* units_left */

static int unit_cols_left (void) {
    return cwp->x_size - cwp->right - cwp->x_cursor + 1; 
} /* cols left */
 ...
 * get_max_width
 *
 * Return maximum width of a line in COLS in the given window. This is used in
...
void screen_new_line (void)
{
    ...
    cwp->x_current_width = (cwp->left*os_char_width(' '))+1;
...
void screen_char (zchar c)
{
    ...
    if (units_left () < (width = os_char_width (c))) {
	   ...
	   if (!enable_wrapping) {
          ...
          cwp->x_current_width = cwp->width - (cwp->right*os_char_width(' '));
          return;
       }
	   screen_new_line ();
    }
    ...
    cwp->x_cursor += os_char_col_width(c);
    cwp->x_current_width += width;
...
{
    int width; /* gfx width */
    int col_width = os_string_col_width(s); /* col width */
    ...
    if (units_left () < (width = os_string_width (s))) {
	   ...
	   if (!enable_wrapping) { /* wrapping disabled OPEN */
	      zchar c;
...
{
    int width; int col_width = os_string_col_width (buf);
...
void screen_erase_input (const zchar *buf)
{
	...
	int width = os_string_col_width (buf);
...
zchar console_read_input (int max, zchar *buf, zword timeout, bool continued)
{
    ...
    cwp->x_cursor -= os_string_col_width (buf);
    key = os_read_line (max, buf, timeout, unit_cols_left (), continued);
    cwp->x_cursor += os_string_col_width (buf);
...
static void erase_screen (zword win)
{
    ...
    /* WRONG! used to be h_screen_width, h_screen_height :( */
    /* os_erase_area (1, 1, h_screen_width, h_screen_height); */
...
void resize_screen (void)
{
	...
	wp[0].x_size = h_screen_cols;
	wp[0].width = h_screen_width;
	wp[1].x_size = h_screen_cols;
    wp[1].width = h_screen_width;
	wp[7].x_size = h_screen_cols;
	wp[7].width = h_screen_width;
...
void restart_screen (void)
{
    ...
    wp[1].x_size = h_screen_cols; /* status bar */
    wp[1].width = h_screen_width;
    wp[0].x_size = h_screen_cols;
    wp[0].width = h_screen_width;
    ...
    */
    ...
    /*
    fprintf (debug, "preparing windows: wp[0].x_size = %d\n", wp[0].x_size);
    fprintf (debug, "preparing windows: wp[1].x_size = %d\n", wp[1].x_size);
    */
...
void z_erase_line (void)
{
    ...
    if (--pixels == 0 || pixels > unit_cols_left ())
	pixels = unit_cols_left ();
    /* FOR PIXELS READ CELLS */
    ...
    /* STILL CHEATING WITH THE FONT HEIGHT */
...
void z_get_cursor (void)
{
    ...
    /* I KILLED THIS. These *are* grid positions. */
    /*
    if (h_version != V6) {
...
static void pad_status_line (int column)
{
    ...
    spaces = unit_cols_left () - column;
    /* while (spaces--)
	screen_char (' '); */
    	
	/* while ((units_left() - os_char_width('W') * column) > 0)
	      screen_char (' '); */

       vis_debug('S', "NOT Padding.");
...
{
     vis_debug('S', "Entering z_show_status...");
     
     fprintf(debug, "[S] h_version %d\n", h_version);
