...
char cell_char(cell c) {return c & 0xff;}
int cell_style(cell c) {return c >> 8;}
...
/* CORRECT! dumb_row uses h_screen_cols to decide what to return */
cell *dumb_row(int r) {return screen_data + r * h_screen_cols;}

cell dumb_cell_val(int r, int c) { return dumb_row(r)[c]; }

/* CORRECT AGAIN! */
static char *dumb_changes_row(int r) { return screen_changes + r * h_screen_cols; }
...
int os_char_width (zchar z) {
  ...
  int w = vis_get_char_width(z);
  return w;
  /* return 1; */
}

int os_char_col_width (zchar z) { return 1; }
...
int os_string_col_width (const zchar *s) {
  int width = 0;
  zchar c;
...
{
  vis_debug('S', "Entering dumb_show_screen...");
  vis_post_redisplay();
  dumb_dump_screen();
}

void dumb_show_screen_old_lame(bool show_cursor)
{   
  vis_debug('S', "Entering dumb_show_screen...");
...
{
  vis_debug('S', "Entering dumb_dump_screen...");
  
  int r;
  for (r = 1; r < h_screen_height; r++)
    show_row(r);
}
...
void dumb_init_output(void)
{
  ...
  /* UPDATE: h_screen_width & height & cols & rows SET in dumb-init */

  /* h_screen_height = h_screen_rows; *//* still cheating with the heights */
  
  /* h_screen_width = h_screen_cols; */
  /* NO.
   * h_screen_width IS FOR GL widths
   * h_screen_cols IS FOR CELLS
  */
  /* h_screen_width = h_screen_cols; */
  ...
  /*
  extern debug;
  fprintf (debug, "[f] dumb_init_output: h_screen_cols=%d\n", h_screen_cols);
  */
  
  /* h_font_width = 1; */
  h_font_width = os_char_width('W'); /* for now, because its quite WIDE */
  h_font_height = 1; /* still cheating with the heights */
